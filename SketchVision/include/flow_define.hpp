...
protected:
  ...
  SketchVision::pkt_code_t code = 0;
  shared_ptr<vector<shared_ptr<SketchVision::basic_packet>>> p_packet_p_seq;
  shared_ptr<vector<size_t>> p_reverse_index;

public:
  basic_flow() {
    p_packet_p_seq =
        make_shared<vector<shared_ptr<SketchVision::basic_packet>>>();
    p_reverse_index = make_shared<vector<size_t>>();
  }

  basic_flow(const decltype(str) str, const decltype(end) end,
             const decltype(code) code,
             const decltype(p_packet_p_seq) p_packet_p_seq,
             const decltype(p_reverse_index) p_reverse_index)
      : str(str), end(end), code(code), p_packet_p_seq(p_packet_p_seq),
        p_reverse_index(p_reverse_index) {}

  basic_flow(const decltype(p_packet_p_seq) p_packet_p_seq,
             const decltype(p_reverse_index) p_reverse_index)
  ...
  bool emplace_packet(const shared_ptr<SketchVision::basic_packet> p,
                      const size_t rid) {
    if (typeid(*p) == typeid(SketchVision::basic_packet_bad)) {
      return false;
    } else {
  ...
  inline auto get_p_reverse_id(void) -> decltype(p_reverse_index) {
    return p_reverse_index;
  }
  inline auto get_p_packet_p_seq(void) -> decltype(p_packet_p_seq) {
    return p_packet_p_seq;
  }
};

class tuple5_flow4 final : public basic_flow {
public:
  SketchVision::tuple5_conn4 flow_id;

  tuple5_flow4(const SketchVision::tuple5_conn4 &flow_id) : flow_id(flow_id) {}

  tuple5_flow4(const SketchVision::tuple5_conn4 &flow_id,
               const decltype(str) str, const decltype(end) end,
               const decltype(code) code,
               const decltype(p_packet_p_seq) p_packet_p_seq,
               const decltype(p_reverse_index) p_reverse_index)
      : flow_id(flow_id),
        basic_flow(str, end, code, p_packet_p_seq, p_reverse_index) {}
