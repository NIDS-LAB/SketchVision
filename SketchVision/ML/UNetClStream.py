...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.class_embed_dim = time_embed_dim
        ...
        # Incerase as needed
        self.packetNum_embedding = nn.Embedding(256, self.class_embed_dim)
    ...
    def forward(self, x, time, label, pNum):

        time_embed = self.time_embedding(time)
        pNum_embed = self.packetNum_embedding(pNum)
        
        if isinstance(label, int):
            label = torch.tensor([label], dtype=torch.long).to(x.device)
        
        if self.class_embedding is not None:
            if label is None:
                null_index = self.num_classes - 1
                label = torch.full((x.shape[0],), null_index, dtype=torch.long, device=x.device)
            class_embed = self.class_embedding(label)
            
        final_embed = time_embed + class_embed + pNum_embed
        
        skips = []

        h = x.type(self.dtype)
        for i, module in enumerate(self.down):
            h = module(h, final_embed)
            skips.append(h)
        h = self.middle(h, final_embed)
